...
import { getFundamentals } from '@/lib/market/server'
...
export async function GET(request: Request, context: { params: Promise<{ symbol: string }> }) {
  ...
  if (!allowRequest(requestKey(request, 'fundamentals'))) return NextResponse.json({ error: 'RATE_LIMITED', requestId }, { status: 429 })
  ...
  try { return NextResponse.json(await getFundamentals(symbol.data), { headers: { 'x-request-id': requestId, 'cache-control': 'private, max-age=300' } }) }
