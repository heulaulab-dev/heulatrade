...
Not yet production-complete: the upstream source provides no trustworthy per-security broker tape, news/announcements feed, historical ownership snapshots, or inputs for broker and technical screener predicates. The current chart supports candles/line/area and EMA overlays but not every PRD mode or indicator. Column resizing, all watchlist and portfolio management operations, storage export workflows, full preferences, and all requested integration coverage are also outstanding. An alert-evaluation endpoint is implemented but still needs a deployed EOD schedule and a real Supabase end-to-end test. The initial Supabase migration has been applied, but the RLS test script has not been executed against a database. A populated `idx-bei` dataset and market-data distribution-rights review are external release prerequisites. Therefore passing local checks does not imply the full PRD acceptance gate has been met.

## Supabase connection update (2026-09-21)

The supplied project URL and publishable key are configured in ignored `.env.local`; the Auth health endpoint returned HTTP 200 with that key. Supabase MCP applied the initial schema to project `rdpusflzpfqrdrrcozav` as migration `20260921002404_initial_schema`. It created 20 public tables with RLS enabled. All seven reference market tables returned HTTP 200 through the publishable Supabase client. Database types were generated from the deployed project and wired into browser and server clients. Real user isolation and browser persistence verification require two confirmed test-user credentials; the live RLS and Playwright suites are prepared for those accounts. The existing Next.js 16 proxy already uses `getClaims()` and propagates refreshed cookies, so the sample `todos` page and duplicate client helpers were intentionally not copied.
